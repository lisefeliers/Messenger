...
#dans aggfunc on peut en mettre plusieurs ( dans le reste aussi grâce à [])
...
# %%
display(df['magnesium'].min())
display(df['magnesium'].max())
display(df['magnesium'].mean())
...
# %%
m = pd.cut(df['magnesium'], bins=[-np.inf,df['magnesium'].mean(),np.inf], labels = ['mag-low', 'mag-high'])
df['mag-cat'] = m
df
...
# %%
g = df.groupby(['Sex', 'Pclass'])['Age']
g.aggregate(['min', 'max'])

# %%
#fait la moyenne en broadcastant => même shape que le dataframe d'origine
df['Age'] - df.groupby(['Sex', 'Pclass'])['Age'].transform('mean')
...
# %%
df.groupby(by = ['Pclass', 'Sex', 'Survived']).size()
...
# %%
g = df.groupby(by = ['Pclass', 'Sex', 'Survived'])
g.get_group((1, 'female', 0))

#deuxieme technique
m = (df['Sex'] == 'female') & (df['Pclass'] == 1) & (df['Survived'] == 0)
df[m]
...
# %%
g = df.groupby(by = ['Sex', 'Pclass'])['Survived'].mean().round(2).to_dict()
g
#to_dict pour mettre en dictionnaire
